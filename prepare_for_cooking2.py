# testpage.py
import os
from typing import List
from hotspots import Hotspot


class PrepareForCookingPage2:
    """
    Page model:
      - uses homepage.png
      - defines three hotspots and their callbacks
    """

    IMAGE_NAME = "03PrepareForCooking.png"

    def __init__(self, controller=None, from_info=False, meal_index=None):
        self.controller = controller
        self.from_info = from_info
        self.meal_index = meal_index

        here = os.path.dirname(__file__) if "__file__" in globals() else os.getcwd()
        assets_dir = os.path.join(here, "assets")
        os.makedirs(assets_dir, exist_ok=True)

        self.image_path = os.path.join(assets_dir, self.IMAGE_NAME)

        self.hotspots: List[Hotspot] = [
            Hotspot(
                "back",
                (84, 642, 154, 707),
                self.on_back_clicked,  # ← capture parameter
            ),
            Hotspot(
                "forward",
                (1122, 642, 1220, 707),
                self.on_forward_clicked,  # ← capture parameter
            ),
        ]

    # ------------------------------------------------------------------
    # Callbacks
    # ------------------------------------------------------------------

    def on_back_clicked(self):
        print("on_back_clicked")
        if self.controller:
            self.controller.show_PrepareForCookingPage1(
                from_info=self.from_info,
                meal_index=self.meal_index,
            )

    def on_forward_clicked(self):
        print("on_forward_clicked")

        if not self.controller:
            return

        if self.from_info:
            self.controller.show_SelectMealPage(from_info=True)
        else:
            self.controller.show_StartCookingConfirmation(
                meal_index=self.meal_index
            )
